...
- **Selective test tooling**: `test-mapping.json` maps source paths to test subdirectories, and `scripts/resolve-affected-tests.mjs` can resolve a changed-file list to focused test paths for local or ad hoc use. Automatic CI runs the full suite after changes reach `main` and on release-please PRs.
...
- CI workflow: `.github/workflows/mock-llm-e2e.yml` runs the full suite on pushes to `main`, same-repository release-please PRs (head branches beginning `release-please--branches--`), and manual dispatch. The `pull_request` trigger remains because classic branch protection requires the `mock-llm-e2e` context; the job is skipped-success for ordinary PRs, following the same pattern as the optional `live-e2e` job. Test results are uploaded as artifacts and rendered in the Actions step summary; the workflow does not post PR comments.
...
- CI workflow: `.github/workflows/mock-llm-docker-e2e.yml` pulls an already-built image from GHCR (no rebuild). It runs automatically after a successful `Docker` workflow on `main`, on same-repository release-please PRs after waiting for that PR's Docker image, and by manual dispatch with an optional custom `docker_image`. Ordinary PRs skip the job. The image tag is derived from the tested commit SHA (`ghcr.io/openhands/agent-canvas:sha-<short>-amd64`). Report artifacts go to `test-results-mock-llm-docker/` and `playwright-report-mock-llm-docker/`, with a summary in the Actions run.

## Debugging E2E Test Failures

When an E2E test fails in CI, use this workflow to diagnose the root cause efficiently:

### 1. Read the Actions summary first
The mock-LLM E2E workflows render a structured Actions step summary with a test results table, pass/fail status, and collapsible failure details including the Playwright error message. **Start here** — the error message usually reveals whether the failure is a locator mismatch, a timeout, or a missing element.
