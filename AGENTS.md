...
- Workspace-session cookie over HTTP (file viewer 401 fix): the agent-server mints the workspace static-fileserver session cookie `oh_workspace_session_key` with `HttpOnly; Secure; SameSite=none; Partitioned`. Browsers silently **drop** a `Secure` cookie on plain-HTTP origins (local dev/static stacks are served over HTTP), so the right-panel file viewer's text fetch (and iframe/`<img>` embeds) carried no credential and the fileserver returned `401 Unauthorized` ("Failed to read <file>: 401"). Two-pronged fix: (1) `useWorkspaceFileContent` (`src/hooks/query/use-workspace-file-content.ts`) text-fetch path now sends `X-Session-API-Key` (resolved from the conversation's `session_api_key` → active backend `apiKey`) alongside `credentials: "include"`, so the request authenticates via header even when the cookie is dropped — the agent-server fileserver accepts either the header or the cookie. (2) The ingress/static-server proxies rewrite the upstream `Set-Cookie` to strip `Secure`/`Partitioned`/`SameSite=none` and add `SameSite=Lax` for plain-HTTP clients, so the cookie actually lands in the browser jar for embeds that can't set headers. The downgrade lives in `downgradeSecureCookie()` + `installSecureCookieDowngrade()` in `scripts/proxy-utils.mjs`, wired in via `createProxyHandlers({ downgradeSecureCookieOverHttp: true })` (default on) so both `ingress.mjs` and `static-server.mjs` benefit; `proxyRes` (httpxy, fires before `webOutgoingMiddleware`) is the injection point. The proxy downgrade must stay HTTP-only (never strip `Secure` on an HTTPS origin) and never be a global override. The frontend header path is the load-bearing fix for text reads; the cookie downgrade covers iframe/`<img>`. Tests: `__tests__/scripts/ingress.test.ts` (downgrade unit + integration via `node:http` `httpRequest`, since jsdom `fetch` filters `set-cookie`) and `__tests__/hooks/query/use-workspace-file-content.test.tsx` (asserts the header).
